...
test_that("parse_description_file", {
  # description file
  ...
  x <- parse_description_file("x", file_data = fd)
  ...
  dat <- c(dat, "x")
  fd <- paste(labels, dat, sep = ":")
  x1 <- parse_description_file("x", file_data = fd)
  expect_identical(colnames(x), colnames(x1))
})

test_that("parse_contributors_file", {
  # contributors file
  ...
  x <- parse_contributors_file("x", file_data = fd)
  expect_is(x, "data.frame")
  expect_equivalent(nrow(x), 1)
  expect_true(all(labels %in% names(x)))
  ...
  fd <- c(paste(labels, collapse = ","), paste(dat, collapse = ","))
  expect_error(parse_contributors_file("x", file_data = fd))
  ...
  fd <- c(paste(labels, collapse = ","), paste(dat, collapse = ","))
  expect_silent(parse_contributors_file("x", file_data = fd))
...
test_that("parse_ports_file", {
  # ports file
  labels <- c("CSR_PORT", "CSR_MSMT_VAR", "CSR_TREATMENT")
  dat <- c("0", "Rs", "None")
  fd <- c(paste(labels, collapse = ","), paste(dat, collapse = ","))

  x <- parse_ports_file("x", file_data = fd)
  expect_is(x, "data.frame")
  expect_equivalent(nrow(x), 1)
  expect_true(all(labels %in% names(x)))
  ...
  for(i in seq_along(labels)) {
    ...
    expect_error(parse_ports_file("x", file_data = fd), info = labels[i])
  }
  ...
  dat <- c("0", "Rz", "None")
  fd <- c(paste(labels, collapse = ","), paste(dat, collapse = ","))
  expect_error(parse_ports_file("x", file_data = fd))
})
